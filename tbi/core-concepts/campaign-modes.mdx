...
- Bob deposits, holds for 20 days, then withdraws to zero on day 21. He forfeits everything he had earned for those 20 days. Even if he re-enters later, the original cliff is broken permanently for that wallet.
...
- A campaign that ends mid-cliff is held open in the background for the cliff length so late-joining users still have a chance to clear theirs.

**Restrictions.** Once a wallet breaks a cliff, that wallet stays excluded for the rest of the campaign. A user wanting a fresh attempt would need to use a different address.
